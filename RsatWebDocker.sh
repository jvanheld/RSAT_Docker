...
DockerStatus=`ps -aef | grep docker | wc -l`
if [ $DockerStatus -eq 1 ]
then
...
NbIfaces=`ip -f inet -o addr | grep -v docker | grep -v 127.0 | wc -l`
if [ $NbIfaces -eq 0 ]
then
	echo "No network interfaces; exit"
	exit
fi
echo "You can connect the Rsat Server using your browser at the url http://"`ip -f inet -o addr | grep -v docker | grep -v 127.0 | awk -F 'inet ' '{print $2}' | awk -F '\/' '{print $1}'`":"$port"/rsat"
if [ -z ${2+x} ]; then MountedDirectory=/rsat_data; else MountedDirectory=$2; fi
if [ -d "$MountedDirectory" ]
then
	echo " "
else
	echo "The directory " $MountedDirectory " does not exist; please create it or give absolute path beginning with /"
	exit
fi
