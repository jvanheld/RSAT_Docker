...
apt install -y dialog git cvs r-base-core net-tools python python-numpy libssl-dev libexpat1-dev libxml2-dev apache2 apt-utils libapache2-mod-perl2 libapache2-mod-perl2-dev php5 perl-doc libdb5.3-dev cpanminus wget sudo libgd-tools libgd-gd2-perl ghostscript gnuplot graphviz mysql-client 
##
...
cd /
RsatVer=`wget -q -O- http://pedagogix-tagc.univ-mrs.fr/download_rsat/ | grep rsat | grep class | awk -F'"' '{print  $6}'`
echo Downloading  $RsatVer
echo $RsatVer | sed 's/rsat_//g' | sed 's/.tar.gz//g' > /RsatInstall/RsatVersion
wget http://pedagogix-tagc.univ-mrs.fr/download_rsat/$RsatVer
mv $RsatVer rsat.tar.gz
gunzip rsat.tar.gz
tar -xvf rsat.tar
##
##
...
##pas de declaration de nom officielle
